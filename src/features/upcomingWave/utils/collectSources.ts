...
export const collectSources = (content: Content): SourceGroup[] => {
  ...
  story.forEach((item) => {
    if (item.kind === 'scene') {
      ...
      if (scene?.band) groups.push({ id: scene.id, title: `${scene.title.lead} ${scene.title.accent}`, sources: [...scene.band.flatMap((block) => blockSources(block, content)), ...(scene.upside?.sources ?? [])] });
    }
    ...
    if (item.kind === 'split') groups.push({ id: split.id, title: `${split.title.lead} ${split.title.accent}`, sources: split.band.flatMap((block) => blockSources(block, content)) });
    if (item.kind === 'calm') groups.push({ id: calm.id, title: `${calm.title.lead} ${calm.title.accent}`, sources: calm.blocks.flatMap((block) => blockSources(block, content)) });
  });
